// Background service worker for RPKI Protection Checker
let rpkiData = null;
...
async function getCacheDuration() {
  try {
    const result = await chrome.storage.local.get(['cacheTimeoutHours']);
...
async function handleFlushCache() {
  try {
    ...
    await chrome.storage.local.remove(['rpkiDataCache', 'rpkiDataTimestamp']);
...
async function handleSetCacheTimeout(hours) {
  try {
    ...
    // Save to storage
    await chrome.storage.local.set({ cacheTimeoutHours: hours });
